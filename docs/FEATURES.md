...
- 🟢 F0812: Add a question mark just right to "Veri Girişi" title. On hover provide a popup that explains user should enter the date of a change in income and amount of new income, no need for repeating the amount for each month and accepted format. Add a similar question mark next to "Kaydet/Yükle" title and in pop-up repeat the information about no data being sent to server, it is saved locally. Decide on the text content, make it concise. Before implementing ask for my feedback for your decision.
- 🟢 F0813: Rename "Veri Girişi" with "Gelir Değişimleri". Add an hide icon(e.g. closed eye) just right to title. When clicked, make the text box used for data entry read only and replace each non-space characters with '*' and keep space chars effectively hiding data and replacing the icon with an open eye. If clicked again, reset the icon, readonly state of textbox and the data in text with original one (so make sure you do not loose the data entered upon hiding it).